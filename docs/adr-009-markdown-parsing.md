...
- These edges are real graph edges and were, until `docs/known_issues/05`, walked by cycle detection like any `import` — so docs cross-linking each other showed up as "circular dependencies". Every edge `parseMarkdown` emits now carries `ImportEdge.isDocReference: true` (preserved through resolution via the builder's `...imp` spread), and `GraphAnalyzer.findCycles` skips those by default (`includeKinds: ["docReference"]` opts them back in). Doc-drift enrichment still consumes them.
