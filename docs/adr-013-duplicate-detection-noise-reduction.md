...
### Phase 5 — generated-file exclusion + import-block stripping: **done** (2026-09-03)

Dogfooding `find_duplicates` against real JVM monorepos (`docs/known_issues/05-...`) surfaced two
noise classes the punctuation gate doesn't touch:

- **Generated / vendored code** — protobuf output, OpenAPI clients, `*.generated.*` — is
  genuinely repetitive by construction and swamps real findings by line count.
  `src/graph/duplication/generated.ts` (`isGeneratedPath` / `hasGeneratedMarker`) recognises it
  by path (`generated/` / `__generated__/` / `__snapshots__/` segments, codegen basename
  suffixes, `*.snap`) or a `@generated` / `DO NOT EDIT` / `Code generated by` / `Jest Snapshot v1`
  banner in the first ~500 bytes. (`__snapshots__`/`.snap` was added after a second dogfood pass
  on box-ui-elements, where uniform snapshot files self-matched ~20× under the sliding window.) Excluded by
  default in `findDuplicates`; `includeGenerated: true` (option / `MokoshConfig.duplication`)
  scans them anyway and tags matches `signals: ["generated"]`. Extra patterns via
  `duplication.ignoreGlobs` (two simple shapes only — `**​/name/**`, `*.suffix`). The cached
  tokenize result (`CachedFileTokens`) gained a `generated` boolean so a cache-hit scan can
  still exclude without re-reading.
- **Shared import blocks** — long identical `import` / `from … import` lists tokenize
  identically. `tokenize()` now masks import/using statements before shingling (per-`FileType`
  `IMPORT_LINE_PATTERNS` plus multi-line handling for JS/TS brace imports and Go `import ( … )`),
  same rationale and same space-fill mechanism as comment stripping.

**`DuplicateGroup` gained `signals?: DuplicateSignal[]`** (`"same-file"` | `"generated"`) — the
coordinated shape change for `docs/known_issues/06` (query layer) and `07` (`kind: "definition"`
groups) to build on. `"crosses-package"` and `"mostly-accessors"` from the issue write-up were
deferred: no package boundary exists on a single `Graph`, and accessor suppression belongs with
issue 7's per-language work.

**Not done, deliberately:** the issue's "minimum distinct-identifier threshold" is the exact
distinct-token-diversity gate Phase 2 already tried and reverted (see above) — and is anyway
unimplementable as stated, since `tokenize()` collapses every identifier to `ID` before any gate
runs.
...
3. `src/mcp/handlers.test.ts` still reports a handful of large clusters of genuinely repeated
   ...
   These now carry `signals: ["same-file"]`, which makes them easy to filter out of a scan.
4. Per-language *definition* duplication (identical TS types, repeated CSS custom properties) —
   `docs/known_issues/07`, which adds `kind: "definition"` groups alongside today's block matches.
