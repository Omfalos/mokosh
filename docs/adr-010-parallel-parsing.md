...
- The originally shipped default (`minFiles: 20`) was well below that crossover — this repo's own 223 files regressed from ~150-220ms (sync) to ~1.6-1.8s (pooled), an ~8x slowdown for a typical small/medium repo. This was measured and shown to @omfalos on 2026-07-14, who initially decided to keep the low default anyway (mokosh's own build stayed acceptably fast either way, and the stated goal was making large repos faster). On 2026-08-13, with the crossover point already diagnosed, @omfalos decided to raise the default to `minFiles: 600` — matching the measured crossover so small/typical repos no longer regress by default.
