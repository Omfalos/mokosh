...
**Returns:** `{ minLines, groups: Array<{ occurrences: Array<{ file, startLine, endLine }>, lines, tokens, family: "style" | "code" }>, count: number }`. Each group has two or more occurrences — every block sharing an identical maximal duplicated run is clustered into one group, so a block duplicated across three files is reported as one three-occurrence group, not three pairs.
