...
Lock files (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`), files under an ignored directory, `type: "unknown"` nodes (non-code assets such as `.svg` / `.json` / images that an explicit `import` pulled into the graph — an icon set is dozens of near-identical `<svg>` files, and they're not source duplication), and generated / vendored files are always excluded, even if the graph itself contains them — `graph.nodes` isn't reliably ignore-rule-filtered for files reached via a resolved reference rather than the initial scan (e.g. a Markdown doc's code-span mention of a build artifact). Generated files are recognised by path (`generated/` / `__generated__/` / `__snapshots__/` directory segments, basenames like `*.pb.go`, `*_pb2.py`, `*.generated.ts`, `*.snap`) or by a `@generated` / `DO NOT EDIT` / `Code generated by` / `Jest Snapshot v1` marker in their first ~500 bytes; pass `includeGenerated: true` (or set `duplication.includeGenerated` / add patterns to `duplication.ignoreGlobs` in config) to change this.

On top of the two block-matching strategies above, two **declaration-level** detectors run independently and contribute `kind: "definition"` groups to the same result (see [ADR-018](./adr-018-per-language-definition-duplicates.md)):

- **CSS/SCSS/Less variables** (`defKind: "cssVar"`) — every custom property (`--x`), SCSS `$variable`, and Less `@variable` declaration is compared two ways: by normalized **value**, reporting groups of ≥2 declaration sites sharing one value regardless of name (a consolidation candidate — works across dialects, e.g. a SCSS `$brand` and a CSS `--brand` holding the same color); and by **name**, reporting a group tagged `signals: ["value-drift"]` when the same variable name holds ≥2 different values across files (an accidental inconsistency, not intentional). A same-name-same-value declaration is only reported once, via the value grouping — it's not drift.
- **TypeScript `interface`/`type` shapes** (`defKind: "interface"` or `"type"`) — object-shaped `interface` and `type` declarations (type-literal aliases only — unions, primitives, `unknown`/`any`, and non-object aliases are skipped) with ≥2 members are canonicalized by sorted `(name, printed type, optional?)` member list and grouped when identical. Member order doesn't affect matching; an `interface` and a `type` alias with the same shape are grouped together, since that's the same "these should be one declaration" signal.
- **`const` object literals** (`defKind: "objectLiteral"`, TS and JS both) — unlike the interface/type shape match above, this compares actual key *and* value content, not just key names: two literals with ≥3 keys match only when every key and its printed value are identical (order-independent). Deliberately stricter than shape matching, since two unrelated config objects sharing key names but different values (`{ host: 'x', port: 80 }` vs `{ host: 'y', port: 443 }`) are not a duplication candidate the way two identically-shaped types are. `let`/`var` declarations, spreads, and computed keys are skipped.
- **JSX/TSX elements** (`defKind: "jsxElement"`, TS and JS both) — every JSX element/fragment (any nesting depth, not just whole-component-return shapes) with ≥40 characters of canonicalized content is compared by tag name, sorted attribute values *as written*, and in-order children. Fixes the "icon component" false positive the token-shingle path can't avoid: under default `ignoreLiterals: true`, the one thing distinguishing two icons — an SVG `d` path string — collapses to a placeholder before matching, so two different icons sharing wrapper boilerplate can token-match. This compares actual attribute content, so only a genuinely copy-pasted element (identical path data, not just identical wrapper) matches. Elements with a spread attribute/child are skipped. A `jsxElement` group whose spans are pure SVG markup — two icons sharing a byte-identical `<defs>`/`<filter>` block — is tagged `signals: ["svg-markup"]` and excluded from the default view (recoverable with `includeSvgMarkup`), same as the token-shingle block matcher's same-skeleton icon false positives.
...
| `limit` | `number` | no | Max clusters/groups in the response, largest-first (default: `20`). Ignored for the default summary view, which is fixed at an 8-cluster preview |
...
| `filter` | `string` | no | `key:value` result filter, AND across keys — see [query.md § find_duplicates filter DSL](./query.md#find_duplicates-filter-dsl). Applied server-side before clustering, so `clusters` narrow with `groups`. With a `filter` and no `view` the response is `summary` + matching `clusters` (no `groups`). An unknown key or malformed clause errors |
| `view` | `string` | no | Which lists the response carries. `"summary"` (default): `summary` + a small `clusters` preview + a `hint`, **no `groups`** — every group in a multi-member cluster is redundant with it. `"groups"`: `summary` + the raw per-span `groups` (+ `count`), no `clusters`. `"full"`: both, with `groups` de-duped against the returned multi-member clusters and `summary.clusteredGroups` reporting the fold count |
| `slim` | `boolean` | no | Compact response (default: `true`). Each group becomes `{ lines, score?, family?, kind?, defKind?, signals?, occurrences: ["path:start-end"] }`; each cluster is `{ files, matchCount, longestMatch, longestMatchAt: ["path:start-end"], coverage }` — `longestMatchAt` (the biggest match's span) makes a cluster actionable without the `groups` list. Pass `false` for the full objects (duplicated source spans, per-occurrence metadata, nested cluster groups) |
...
Each cluster also carries `coverage` — one entry per file in `files`, with that file's occurrence spans merged (overlapping/adjacent sub-matches counted once, not once per group) and divided by the file's total line count when known. `matchCount: 14` on its own still reads as "14 things"; `coverage` turns it into `"ContentExplorer.tsx: 62% (312/501 lines) duplicated with ContentPicker.js"` — high coverage on both sides suggests near-duplicate files, low coverage suggests one shared helper embedded in two otherwise-unrelated files. `totalLines`/`coveragePct` are `undefined` when the file's line count wasn't available (a token-cache hit that skipped re-reading the source).
