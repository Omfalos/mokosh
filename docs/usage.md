...
| `tags` | `{ blocklist?: string[], allowlist?: string[] }` | Tunes which tags count as **test-selection labels** for `--propose-tags` / `propose_tags`, `--apply-tags`, `--list-tags` (default view) and the `tag:` query filter. `blocklist` names are treated as noise on top of the built-in curated list; `allowlist` keeps a name even if built-in- or user-blocked. Both case-insensitive. See [Test Tags](./test-tags.md#tag-quality). |
| `parallelParsing` | `boolean \| { minFiles?, maxThreads? }` | Controls worker-pool offloading of file parsing. Default (`true`/unset) enables it once the repo has at least `minFiles` (default `600`) files, matching the measured crossover point where pooling actually pays off. Set `false` to always parse in-process, or pass `{ minFiles, maxThreads }` to lower/raise the threshold for your own repo. See [ADR-010](./adr-010-parallel-parsing.md). |
| `pathAliases` | `Record<string, string[]>` | Explicit path-alias map for import resolution, same shape as tsconfig's `compilerOptions.paths` (e.g. `{ "@app/*": ["src/app/*"] }`). Takes precedence over any `tsconfig.json` aliases. Useful for JS-only projects without a tsconfig, or to mirror Vite/webpack alias configs. |
