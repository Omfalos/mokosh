...
import { compareBranches, configToGraphOptions, summarizeBranchComparison } from "../../index";
 ...
 *   duplication?" and "did every call site of a renamed export get updated?". Prints the compact
 *   summary by default; `--compare-full` prints the complete `BranchComparison`, `--compare-max-items`
 *   tunes how many entries each summary delta list keeps.
 ...
 *   underlying tool calls; `ctx.compareFull`/`ctx.compareMaxItems` shape the output.
 */
export async function run(ctx: CommandContext): Promise<void> {
  ...
  const output = ctx.compareFull
    ? comparison
    : summarizeBranchComparison(comparison, {
        metric: ctx.metric,
        maxItems: ctx.compareMaxItems,
      });
