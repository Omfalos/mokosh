...
import { buildResponsibilityGraph } from "./responsibility";
...
describe("buildResponsibilityGraph", () => {
  ...
  test("infers role: mcp for files in /mcp/ path", () => {
    const graph = makeGraph([makeNode("src/mcp/server.ts", "logic")]);
    const rg = buildResponsibilityGraph(graph);
    expect(rg.get("src/mcp/server.ts")?.role).toBe("mcp");
  });
  ...
  test("infers role: workspace for files in /workspace/ path", () => {
    const graph = makeGraph([makeNode("src/graph/workspace/index.ts", "logic")]);
    const rg = buildResponsibilityGraph(graph);
    expect(rg.get("src/graph/workspace/index.ts")?.role).toBe("workspace");
  });
